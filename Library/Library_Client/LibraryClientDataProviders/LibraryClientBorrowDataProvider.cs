...
{
    public static class LibraryClientBorrowDataProvider
    {
        ...
        public static void DeleteBorrow(long id)
        {
            using (var client = new HttpClient())
            {
                var response = client.DeleteAsync(new Uri(_uniformResourceIdentifier + "/" + id.ToString(CultureInfo.InvariantCulture))).Result;
                if (!response.IsSuccessStatusCode)
                {
                    throw new InvalidOperationException(response.StatusCode.ToString());
                }
